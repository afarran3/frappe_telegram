...
def get_telegram_user(update: Update):
    ...
    user = frappe.db.get_value("Telegram User", {"telegram_user_id": telegram_user.id},['name', 'user', 'full_name'], as_dict=True)
    if user:
        if user.user and str(user.name) != str(str(user.full_name) + "-(" + str(user.user) + ")"):
            frappe.rename_doc("Telegram User", user.name, str(str(user.full_name) + "-(" + str(user.user) + ")"))
        return frappe.get_cached_doc("Telegram User", user.name)
