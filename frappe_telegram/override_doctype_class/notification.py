...
def get_recipients(notification, doc, context):
    ...
    for recipient in notification.recipients:
        ...
        if recipient.receiver_by_telegram_user:
            user = frappe.db.get_value("Telegram User", {"name": recipient.receiver_by_telegram_user}, "user")
            if user:
                if frappe.db.exists("User", user) and frappe.db.get_value("User", {"name": user}, "enabled") == 1:
                    recipients.append(user)
